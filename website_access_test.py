...
def main():
    ...
    log(f"   可访问网站：{len(accessible_websites)}")
    log(f"   不可访问网站：{len(inaccessible_websites)}")
    
    log(f"\n💡 下一步：")
    log(f"   1. 分析“可访问网站”，用于未来的搜索和抓取")
    log(f"   2. 分析“不可访问网站”，尝试其他方式访问（如 API、代理）")
    log(f"   3. 更新“上网搜索能力”，记录哪些网站可以访问")
